...
if(isset($_POST['submit'])) {
    ...
    if(empty($_POST['username']) OR empty($_POST['email']) OR empty($_POST['password'])) {
        echo "<script>alert('One or more inputs are empty'); window.location.href='register.php';</script>";
    } else {
        ...
        if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/', $password)) {
            echo "<script>alert('Password must be at least 8 characters long and include one uppercase letter, one lowercase letter, and one digit.'); window.location.href='register.php';</script>";
            exit();
        }
        else {
          ...
          echo "<script>alert('Successfully registered! Redirecting to login page...'); window.location.href='login.php';</script>";
          exit();
        }
    }
}
